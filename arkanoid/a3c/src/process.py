    ...
    while True:
       ...
       #print(total_loss)
        ...
        if curr_episode % 100 == 0:
            ...
            "{:.2f}".format(info["score"]),"{:.4f}".format(tot_reward)]
...
def local_test(index, opt, global_model):
    ...
    print(savefile)
    title = ['Steps', 'Time', 'Reward','TotalReward','Score']
    with open(savefile, 'w', newline='') as sfile:
        writer = csv.writer(sfile)
        writer.writerow(title)
    ...
    while True:
        ...
        if done:
            ep_time = time.time() - start_time
            data = [curr_step, "{:.4f}".format(ep_time), "{:.4f}".format(reward), "{:.2f}".format(tot_reward),"{:.2f}".format(info["score"])]
            with open(savefile, 'a', newline='') as sfile:
                writer = csv.writer(sfile)
                writer.writerows([data])
