...
def get_args():
    ...
    parser.add_argument("--log_path", type=str, default="tensorboard/ppo_castlevania_")
...
def train(opt):
    ...
    opt.saved_path = os.getcwd() + '/castlevania/PPO/' + opt.saved_path
    ...
    while True:
        ...
        if tot_loops % opt.save_interval == 0 and tot_loops > 0:
            torch.save(model.state_dict(), "{}/PPO_castlevania_{}_{}".format("trained_models", opt.world, opt.stage))
            torch.save(model.state_dict(), "{}/PPO_castlevania_{}_{}_{}".format("trained_models", opt.world, opt.stage, tot_loops))
      ...
      #  any_flags = np.sum(flags)
        ...
        data = [tot_loops, tot_steps, "{:.6f}".format(ep_time), "{:.4f}".format(avg_loss), "{:.4f}".format(mu_reward), "{:.4f}".format(std_reward), "{:.2f}".format(sum_reward)]
