...
class Behaviour(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(5*128, 6, kernel_size=3, stride=4, padding=1)
...
def validation_and_plots(feat_extract, net, val_loader, device, prefix='./plots'):
    ...
    with torch.no_grad():
        for i, data in enumerate(val_loader, 0):
            ...
            probas_y.extend(pred_probs.data.cpu().numpy().tolist())
            pred_y.extend(predicted.data.cpu().numpy().tolist())
...
if __name__ == "__main__":
    ...
    # ********************************
    # Behaviour for A!
    dataset_train = DatasetMarioBxv2(file_path="./", csv_name="./bx_data/gerardo120719v2_ALL.csv", buttontrain='a', transform_in=transform2apply)

    dataset_val = DatasetMarioBxv2(file_path="./", csv_name="./bx_data/gerardo120719v2_ALL.csv", buttontrain='a', transform_in=transform2apply)
    
    train_loader = create_datasets(dataset_train, shuffle_dataset, batch_training)
    validation_loader = create_datasets(dataset_val, False, batch_validation)
