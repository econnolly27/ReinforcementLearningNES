...
class Behaviour(nn.Module):
    def __init__(self):
        ...
        # self.net_behaviour = nn.Sequential(
    ...
    def forward(self, x):
        ...
        x2 = self.elu(self.do(self.fc1(x1)))
        # x1 = self.bn2(x1)
        output = self.fc2(x2)
        # output = self.fc3(x3)
    ...
    def loss_function(x_net, x, criterion=None):
        if criterion is None:
            criterion = nn.BCEWithLogitsLoss()#nn.CrossEntropyLoss()#nn.NLLLoss()#BCELoss()
...
def validation_and_plots(feat_extract, net, val_loader, device, prefix='./plots'):
    ...
    with torch.no_grad():
        for i, data in enumerate(val_loader, 0):
            ...
            predicted = (output > t.to(device)).float() * 1
...
def trainNetBx(net, feat_extract, train_loader, val_loader, n_epochs, learning_rate, device, namebx):
    ...
    for epoch in range(n_epochs):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            t = Variable(torch.Tensor([0.5]))  # threshold
            predicted = (output_control > t.to(device)).float() * 1
            predicted = torch.flatten(predicted)
            ...
            if (i + 1) % (print_every + 1) == 0:
                print('Epoch [{}/{}] {:d}%, Step [{}/{}], Loss: {:.4f}, Accuracy: {:.2f}% took: {:.2f}s with LR: {:.5f}'
                  .format(epoch + 1, n_epochs, int(100 * (i+1) / n_batches), i + 1, len(train_loader), train_loss / print_every, (correct / total) * 100, time.time() - start_time, learning_rate))#, scheduler.get_lr()[0]))
...
if __name__ == "__main__":
    ...
    trainNetBx(bxmodel, feat_extract, train_loader, validation_loader, n_epochs=50, learning_rate=1e-3, device=device, namebx='bxA')
    # ********************************
