...
class WarpFrame(gym.ObservationWrapper):
    ...
    def observation(self, frame):
        ...
        frame = np.array(frame).astype(np.float32).reshape(1, self.width, self.height)
        return frame
...
class FrameStack(gym.Wrapper):
    def __init__(self, env, num_stack):
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=(shp[0], shp[1], shp[2] * num_stack), dtype=env.observation_space.dtype)
