...
def trainNetBx(net, feat_extract, train_loader, val_loader, n_epochs, lr, device, namebx):
    ...
    for epoch in range(n_epochs):
        ...
        total_val_loss, valmu, valstdev, valvar = validation_and_plots(feat_extract, net, val_loader, device)
        stats_val.append([epoch, valmu, valstdev, valvar])
        if prev_loss > total_val_loss:
            torch.save(net.state_dict(), './models/bestmodel_'+ namebx + str(net.noframes) + 'f.pth')
