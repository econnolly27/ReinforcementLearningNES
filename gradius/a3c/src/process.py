...
def local_train(index, opt, global_model, optimizer, save=False):
    ...
    savefile = opt.saved_path + '/gradius_a3c_train' + opt.timestr + '.csv'
        ...
        if save:
            if curr_episode % opt.save_interval == 0 and curr_episode > 0:
                torch.save(global_model.state_dict(),
                           "{}/a3c_gradius_{}_{}".format(opt.saved_path, opt.world, opt.stage))
                torch.save(global_model.state_dict(),"{}/a3c_gradius_{}_{}_{}".format(opt.saved_path, opt.world, opt.stage,curr_episode))
...
def local_test(index, opt, global_model):
  ...
  #  savefile = opt.saved_path + '/A3C_test' + opt.timestr +  '.csv'
   ...
   # with open(savefile, 'w', newline='') as sfile:
    #    writer = csv.writer(sfile)
     #   writer.writerow(title)
    savefile = opt.saved_path + '/gradius_a3c_test' + opt.timestr + '.csv'
    print(savefile)
    title = ['Steps', 'Time', 'TotalReward', "Flag"]
    with open(savefile, 'w', newline='') as sfile:
        writer = csv.writer(sfile)
     ...
     #   if flag_get(info):
       ...
       #     torch.save(local_model.state_dict(),
        ...
        if curr_step > opt.num_global_steps:
            ...
            torch.save(local_model.state_dict(),
                       "{}/a3c_gradius_{}".format(opt.saved_path, curr_step))
